package sheg1_steparm.aquaacrobaticsunofficial.biome;
...
public abstract class BiomeWaterFogColors {
    ...
    private static void processStringColor(String colorEntry) {
        ...
        if (fields.length != 3) {
            AquaAcrobatics.LOGGER.error("Incorrect syntax for '{}'. Should be modname:biome,color,fogcolor (color and fogcolor may be empty)", colorEntry);
            return;
        }
    ...
    public static int getWaterColorForBiome(Biome biome, int oldColor) {
        ...
        if (oldColor != DEFAULT_WATER_COLOR_112) {
            AquaAcrobatics.LOGGER.info("Potentially missing water color mapping for {}, attempting to fake old appearance", location);
